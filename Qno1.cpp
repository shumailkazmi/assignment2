...
int longstr(string arr[]) { //function to find longest string
...
int main() {
	int len = 0;
	string arr[7];
	readfile(arr);//to read the input file given by the user
	len = longstr(arr);//to check if any word is longer than the standard size of the matrix
	if (len < 10) {
		len = 10;
	}
